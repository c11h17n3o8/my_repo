...
for intf, vlans in trunk.items():
    ...
    for command in trunk_template:
        if command.endswith('allowed vlan') and vlans[0] == "add":
            print(" " + command + " add " + str(vlans[1:]).strip("[]").replace("'",""))
        elif command.endswith('allowed vlan') and vlans[0] == "del":
            print(" " + command + " remove " + str(vlans[1:]).strip("[]").replace("'",""))
        elif command.endswith('allowed vlan') and vlans[0] == "only":
           print(" " + command + " " + str(vlans[1:]).strip("[]").replace("'",""))
        else:
            print(" "+ command)
